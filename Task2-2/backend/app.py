...
BASE_DIR = os.path.abspath(os.path.dirname(__file__)) #to load base and everything else from "frontend" folder
...
def delete_file():
    ...
    try:
        response = requests.post(delete_url, headers=headers, json={"path": file_path})

        if response.status_code == 200:
            result = response.json()
            return jsonify({"message": f'File deleted: {result["metadata"]["name"]}'})
        else:
